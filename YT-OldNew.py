...
def ensure_env():
    ...
    if miss:
        for k in miss: log(f"❌ Отсутствует переменная окружения: {k}")
        raise SystemExit(1)
...
SCOPES_SA = ["https://www.googleapis.com/auth/spreadsheets",
...
def youtube_service(token_file: str):
    if not os.path.exists(token_file):
        raise RuntimeError("Отсутствует token.pickle")
    ...
    if not getattr(creds, "valid", False):
        if getattr(creds, "expired", False) and getattr(creds, "refresh_token", None):
            ...
            raise RuntimeError("Невозможно обновить OAuth токен. Обновите YOUTUBE_TOKEN_B64.")
...
def delete_first_row_with_sheet(sh, sheet_name):
    ...
    for s in meta.get("sheets", []):
        if s["properties"].get("title") == sheet_name:
            sid = s["properties"].get("sheetId"); break
    if sid is None: raise ValueError(f"Sheet '{sheet_name}' not found")
...
def _ensure_valid_video(path: str):
    ...
    if size < 200 * 1024: raise ValueError("Слишком маленький файл")
...
def resolve_video_source(src: str):
    ...
    if src.startswith("http://") or src.startswith("https://"):
        r = requests.get(src, stream=True, timeout=300)
        return _ensure_valid_video(_save_stream_to_tmp(r)), True
...
def upload_video(yt, file_path: str, title: str, description: str) -> str:
    ...
    if YOUTUBE_DEFAULT_TAGS: snippet["tags"] = YOUTUBE_DEFAULT_TAGS
    ...
    while response is None:
        try: _, response = request.next_chunk()
            ...
            log(f"Повтор загрузки: {e}"); time.sleep(3)
            continue
...
def webhook(token):
    ...
    if not chat_id or not text: return jsonify({"ok": True}), 200
